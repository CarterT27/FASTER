...
class StatisticalEvaluator:
    ...
    def _evaluate_single_feature(
        ...
        if isinstance(feature_data, pd.DataFrame):
            if feature_data.shape[1] == 1:
                ...
                self.logger.warning(
                    f"Feature {feature_name} is a multi-column DataFrame. Using first column for evaluation."
                )
                feature_data = feature_data.iloc[:, 0]

        is_categorical = feature_data.dtype == "object" or feature_data.dtype.name == "category"
        is_categorical = is_categorical or (
            isinstance(feature_data, pd.Series) and feature_data.str.contains("bin_").any()
            if hasattr(feature_data, "str")
            else False
        )
        ...
        if is_categorical and self._is_classification_problem(target):
            correlation, p_value, effect_size = self._categorical_vs_categorical(
                feature_data, target
            )
            test_method = "chi_square"
        elif is_categorical and not self._is_classification_problem(target):
            correlation, p_value, effect_size = self._categorical_vs_continuous(
                feature_data, target
            )
            test_method = "anova"
        elif not is_categorical and self._is_classification_problem(target):
            correlation, p_value, effect_size = self._continuous_vs_categorical(
                feature_data, target
            )
        ...
        if p_value > self.alpha and mi_score > 0.2:
            warnings.append(
                f"High mutual information but non-significant p-value, may indicate non-linear relationship"
            )
    ...
    def _categorical_vs_continuous(
        ...
        if isinstance(feature, pd.DataFrame):
            if feature.shape[1] == 1:
                ...
                self.logger.warning(
                    f"Multi-column categorical feature provided. Using first column."
                )
                feature = feature.iloc[:, 0]
        ...
        try:
            if pd.api.types.is_categorical_dtype(feature) or pd.api.types.is_object_dtype(feature):
                if (
                    hasattr(feature, "str")
                    and isinstance(feature.iloc[0], str)
                    and feature.str.contains("bin_").any()
                ):
                    try:
                        numeric_feature = pd.Series(
                            [
                                int(val.split("_")[1])
                                if isinstance(val, str) and "bin_" in val
                                else float("nan")
                                for val in feature
                            ],
                            index=feature.index,
                        ...
                        self.logger.warning(
                            f"Could not convert binned feature to numeric: {str(e)}"
                        )
    ...
    def _run_anova(
        self, numeric_feature: pd.Series, target: pd.Series
    ) -> Tuple[float, float, float]:
        ...
        for stat in results:
            ...
            try:
                if is_classification:
                    base_score = (
                        data[target_column].value_counts(normalize=True).max()
                ...
                if is_classification and scoring == "accuracy":
                    normalized_score = (
                        (avg_score - base_score) / (1 - base_score) if avg_score > base_score else 0
                    )
    ...
    def _detect_categorical_columns(data: pd.DataFrame) -> List[str]:
        ...
        for column in data.columns:
            ...
            if data[column].dtype == "object" or data[column].dtype.name == "category":
                ...
                if unique_vals < min(
                    10, len(data) * 0.05
                ):  # 10 or 5% of rows, whichever is smaller
                    categorical_cols.append(column)
