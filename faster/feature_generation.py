...
class TransformationMetadata:
    ...
    performance_gain: Optional[float] = (
        None  # Improvement in predictive performance from this feature
    )
...
class FeatureGenerator:
        ...
        if categorical_columns is None:
            ...
            for col in data.columns:
                ...
                if (
                    ...
                    or (pd.api.types.is_numeric_dtype(data[col]) and data[col].nunique() < 10)
                ):
                    categorical_columns.append(col)

        try:
            feature_importance = {
                insight.feature_name: insight.importance for insight in domain_insights
            }
            ...
            result_df = self._apply_basic_transformations(
                result_df, domain_insights, target_column, top_features
            )

            result_df = self._generate_interaction_features(
                result_df, domain_insights, target_column, is_classification
            )

            result_df = self._apply_domain_transformations(
                result_df, domain_insights, recommended_transforms
            )
            ...
            if target_column:
                result_df = self._evaluate_transformations(
                    result_df, data, target_column, is_classification
                )
    ...
    def _apply_basic_transformations(
        ...
        for col in numeric_cols:
            ...
            if self._should_apply_log_transform(data[col]) and (
                ...
                if valid_data.min() < 0:
                    logger.warning(
                        f"Column {col} contains negative values. Adjusting before log transformation."
                    )
                ...
                self.transformations[f"log_{col}"] = TransformationMetadata(
                    ...
                    rationale=(
                        ...
                        if "log" not in recommended
                        else "Log transform suggested by domain knowledge"
                    )
                    + transform_note,
                )
    ...
    def _generate_interaction_features(
        ...
        for insight in sorted_insights:
            ...
            if (
                "multiply" in insight.suggested_transformations
                or "interaction" in insight.suggested_transformations
            ) and valid_relationships:
                for related in valid_relationships[: min(3, len(valid_relationships))]:
                    ...
                    if (
                        interaction_name in created_interactions
                        or reverse_name in created_interactions
                    ):
                        continue
        ...
        for insight in sorted_insights[:5]:  # Only consider top 5 insights
            ...
            if any(t.startswith("poly") for t in insight.suggested_transformations):
                poly_features = [insight.feature_name] + [
                    rel
                    for rel in insight.relationships[:2]  # Limit to top 2 relationships
                    if rel in data.columns and (not target_column or rel != target_column)
                ]
                ...
                try:
                    if transform.startswith("log") and feature not in transformed_features:
                        if data[feature].min() > 0:
                            ...
                            if valid_data.min() < 0:
                                logger.warning(
                                    f"Column {feature} contains negative values. Adjusting before log1p transformation."
                                )
                        ...
                        try:
                            result_df[new_feature_name] = self._apply_binning(
                                data[feature], n_bins=n_bins
                            )
                            transformed_features.add(feature)
                        ...
                        result_df[new_feature_name] = (data[feature] - data[feature].mean()) / data[
                            feature
                        ].std()
                        transformed_features.add(feature)
                        self.transformations[new_feature_name] = TransformationMetadata(
                        ...
                        result_df[new_feature_name] = (data[feature] - data[feature].min()) / (
                        ...
                        try:
                            ...
                            if not encoded_df.empty:
                                ...
                                logger.warning(
                                    f"One-hot encoding produced no columns for {feature}"
                                )
                        except Exception as e:
                            logger.warning(
                                f"Error applying one-hot encoding to {feature}: {str(e)}"
                            )
    ...
    def _generate_text_features(
        ...
        text_insights = [
            insight
            for insight in insights
            if insight.feature_name in text_cols
            and any(
                t.startswith("text") or t.startswith("tfidf") or t.startswith("nlp")
    ...
    def _evaluate_transformations(
        ...
        try:
            ...
            eval_metric = (
                ...
                else ("error" if is_classification else "rmse")
            )
            ...
            try:
                ...
                if "early_stopping_rounds" in str(e):
                    ...
                    baseline_model.fit(
                        X_train, y_train, eval_set=[(X_val_orig, y_val)], verbose=False
                    )
                else:
            ...
            for transform_type, cols in transform_groups.items():
                ...
                try:
                    ...
                    logger.info(
                        f"{transform_type} transformation performance: {score:.4f} (improvement: {improvement:.4f})"
                    )
                except Exception as e:
            ...
            for t_type, improvement in sorted(
                transform_performance.items(), key=lambda x: x[1], reverse=True
            ):
    ...
    def _apply_binning(self, series: pd.Series, n_bins: int = 5) -> pd.Series:
        ...
        try:
            ...
            binned = pd.cut(
                series, bins=n_bins, labels=False, include_lowest=True, duplicates="drop"
            )
