...
def plot_metrics_comparison(
    ...
    for metric in metrics:
        ...
        plot_data.append({"Metric": metric.upper(), "Model": "Baseline", "Value": baseline_value})
        plot_data.append(
            {"Metric": metric.upper(), "Model": "FASTER (No Domain)", "Value": no_domain_value}
        )
        plot_data.append(
            {"Metric": metric.upper(), "Model": "FASTER (With Domain)", "Value": with_domain_value}
        )
...
def plot_feature_importance(
    feature_importances: Union[Dict[str, float], pd.Series], top_n: int = 15
) -> plt.Figure:
    """
